...
param __router_param = {"eth1", "eth2", 1, "192.168.0.254"};
...
int send_icmp_time_exceed(int device_no, struct ether_header *eh, struct iphdr *iphdr, u_char *data, int size)
{
...
int analyze_packet(int device_no, u_char *data, int size)
{
    u_char *ptr;
    int rest;
...
printf("%u %s : in\n", __LINE__, __func__);
    ...
    else if (ntohs(eh->ether_type) == ETHERTYPE_IP)
    {
        ...
        if ((iphdr->daddr & __device[device_no].netmask.s_addr) == __device[tno].subnet.s_addr)
        {
            ...
            debug_printf("[%d]:%s to target segment\n", device_no, inet_addr_t2str(iphdr->daddr, buf, sizeof(buf)));
        ...
        else
        {
            ...
            ip2mac = ip_to_mac(tno, __next_router.s_addr, NULL);
            if (ip2mac->flag == FLAG_NG || ip2mac->sd.dno != 0) {
                debug_printf("[%d] ip2mac error or sending\n", device_no);
        ...
        memcpy(eh->ether_shost, __device[tno].hwaddr, MACADDRLEN);
...
int main(int argc, char **argv, char **envp)
{
    ...
    if (argc == 3) {
        __router_param.device1 = strdup(argv[1]);
        __router_param.device2 = strdup(argv[2]);
    }
    ...
    debug_printf("[0] %s Ok\n", __router_param.device1);
    debug_printf("hwaddr=%s\n", my_ether_ntoa_r(__device[0].hwaddr, buf, sizeof(buf)));
    ...
    if (get_device_info(__router_param.device2, __device[1].hwaddr, &__device[1].addr, &__device[1].subnet, &__device[1].netmask) == -1) {
        debug_printf("get_device_info error %s\n", __router_param.device2);
        return -1;
    }
    if ((__device[1].sock = init_raw_socket(__router_param.device2, 0, 0)) == -1) {
        debug_printf("inti_raw_socket error %s\n", __router_param.device2);
        return -1;
    }
    debug_printf("[1] %s Ok\n", __router_param.device2);
    debug_printf("hwaddr=%s\n", my_ether_ntoa_r(__device[1].hwaddr, buf, sizeof(buf)));
